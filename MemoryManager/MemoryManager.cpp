...
//CITE: https://stackoverflow.com/questions/38892455/initializing-an-array-of-zeroes
//CITE: https://stackoverflow.com/questions/19688833/iterate-over-full-array-starting-at-any-position-using-only-one-variable
void *MemoryManager::getBitmap(){

    uint16_t* arr = static_cast<uint16_t *>(getList());

    uint16_t sizeofarr = arr[arr[0]*2-1]+arr[arr[0]*2];

    int remainder = sizeofarr%8;

    string arrayStr = "";

    //assign sizeofarr if theres a remainder
    if(sizeinwords%8 != 0){
        sizeofarr = sizeofarr+(8-remainder);
    }

    uint16_t bs[sizeofarr]= {0};

    uint16_t numBits = sizeofarr/8;
    
    for(int i=1; i<=arr[0]*2; i++){
        if(i%2 == 0){
            for(int j=arr[i-1]; j<arr[i]+arr[i-1]; j++){
                bs[j] = 1;
            }
        }
    }
    ...
    for(int i=0; i<sizeofarr; i++){
        arrayStr += to_string(bs[i]);
    }

    for(int i=0; i<arrayStr.size(); i++){
        if(arrayStr[i] == '1'){
            arrayStr[i] = '0';
        }else if(arrayStr[i] == '0'){
            arrayStr[i] = '1';
        }
    ...
    //CITE: https://stackoverflow.com/questions/50972196/how-to-reverse-a-substring-of-a-string-in-c

    for(uint16_t i=0; i<numBits; i++){
        reverse(arrayStr.begin()+offset, arrayStr.begin() + offset + 8);
        offset += 8;
    }

    //CITE: https://www.geeksforgeeks.org/program-decimal-binary-conversion/
    string numBitsStr;
    while(numBits){
        if(numBits & 1){
            numBitsStr += '1';
    ...
    while(numBitsStr.length() != 16){
        numBitsStr = '0'+numBitsStr;
    }

    //CITE: https://stackoverflow.com/questions/8468597/prepend-stdstring
    for(int i=0; i<8; i++){
        numBitsStr.insert(numBitsStr.length(), 1 , numBitsStr[0]);
        numBitsStr.erase(0,1);
    }

    arrayStr.insert(0, numBitsStr);

    //CITE: https://stackoverflow.com/questions/2204176/how-to-initialise-memory-with-new-operator-in-c

    uint16_t tracker = 0;

    //CITE: https://www.geeksforgeeks.org/program-binary-decimal-conversion/?ref=next_article

    //for every byte...

    size_t sizeofmap = (arrayStr.size())/8;
    uint8_t* finalBitMap = new uint8_t[sizeofmap]();

    for(int i=0; i<arrayStr.size() && i/8 < sizeofmap; i+=8){
        
        //every group of 8 characters...
        string sub = arrayStr.substr(i, 8);
    ...
    uint8_t* FINALfinalBitMap = new uint8_t[sizeofmap]();

    copy(finalBitMap, finalBitMap, FINALfinalBitMap);

    delete[] finalBitMap;
    delete[] arr;

    return (uint8_t*)FINALfinalBitMap;
